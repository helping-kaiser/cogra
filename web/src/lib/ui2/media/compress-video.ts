...
// no H.264 or AAC encoder at the clip's configuration, an encoder that fails
...
// `isConfigSupported` the library documents as unreliable. `Conversion.init`
...
export async function compressVideo(file: Blob, capBytes: number): Promise<CompressResult> {
  ...
  try {
    ...
    const plan = planVideo(probed.clip, capBytes);
    if (plan.kind === "remux") return keep("within-target", "already H.264 + AAC within target");
    ...
    const refused = await capability(plan, probed.audio, videoQuality, audioQuality);
    if (refused !== null) return keep("unsupported", refused);

    let blob: Blob;
    try {
      blob = await encode(input, plan, videoQuality, audioQuality);
    } catch (error) {
    ...
    console.info(
      `${TAG} encoded (${plan.reason}) to ${plan.size.width}x${plan.size.height} at ` +
        `${plan.videoBps} bps: ${file.size} -> ${blob.size} bytes`,
    );
...
/**
 * Null when this browser can encode this clip at the target, or why it cannot.
 *
 * Both halves are required: a clip with sound needs H.264 AND AAC, and a
 * silent clip needs only the picture.
 */
async function capability(
  ...
  if (!(await canEncodeVideo("avc", { width, height, quality: videoQuality }))) {
    return `no H.264 encoder for ${width}x${height} at ${plan.videoBps} bps`;
  }
  if (audio === null) return null;

  const numberOfChannels = await audio.getNumberOfChannels();
  const sampleRate = await audio.getSampleRate();
  if (await canEncodeAudio("aac", { numberOfChannels, sampleRate, quality: audioQuality })) {
    return null;
  }
  if (await canEncodeAudio("aac", { ...FALLBACK_AUDIO, quality: audioQuality })) return null;
  return `no AAC encoder at ${AUDIO_BPS} bps (${numberOfChannels} ch, ${sampleRate} Hz)`;
}

/**
 * The encode itself: one `Conversion`, both tracks forced through the encoder.
 *
...
async function encode(
  ...
  const conversion = await Conversion.init({
    ...
    audio: { codec: "aac", quality: audioQuality, forceTranscode: true },
    tags: {},
