...
export function createKeyCeremony(deps: {
  ...
  async function publicIdentity(): Promise<ActorPublicIdentity | null> {
    ...
    return { publicKeyBase64: toBase64(publicKey), realizationAddress: await addressOf(publicKey) };
  }
  ...
  return {
    ...
    async attachActorKey() {
      ...
      return guard.run(() => attachActorKey(client, identity.publicKeyBase64, identity.realizationAddress));
    },
