...
- **`own` changes the row, not the layout.** Your own profile has no opinion to give, so the row is `Edit profile` and `Invites` sharing the width; settings stays the screen's top-bar gear; applicant versus member is expressed in cards below, never as a different header.
