...
- **The opinion on the person leads the actions row** — the wide anchor, taking whatever the row has left. `Message` stands beside it at the width of its own word, and the page's `menu` closes the row: since the band law no band carries a ⋮, so a profile's rarer acts (mention, share, save, hide) hang off the row that holds its other acts.
...
- **The figures, each labelled, never merged.** Posts leads; then "Opinions on them" and "Opinions by them" — **those three labels, on these figures** (backlog item 88, ruled the batch-rulings round). The two directions are different facts, so a label has to say which one it counts, and one merged "followers" figure would describe a different product: a one-way link that carries no opinion, which is not what a CoGra edge is. The steer is about accuracy on these labels, not about the words themselves — *connection* and *network* are ordinary English and readme §3 welcomes them used plainly. The figures are one tap target (`onCounts`) toward the opinions page, both directions separated there too.
...
- **`invitesWaiting` puts the bell's dot on `Invites`** (the invites round). An application that has become approvable waits on this person and nobody else, so the row that opens the queue is the one place it can be learned. A DOT, never a count — the bell's own ruling reused rather than a second badge vocabulary invented beside it: the button says *something is waiting*, and the list it opens is where a number means anything. The accessible name changes with it (`Invites — someone is waiting`), because a marker a listener cannot hear is not a marker.
