...
```jsx
<WalletBalance amount={128.4} approx="0.00087" delta={14.4} onHelp={openWhatIsCgt} />
```
...
- The "?" is *What is CGT?*; the ≈ line reads the public CGT–L-BTC market (an estimate, never a promise, hidden at zero); `delta` is real recent earnings as an AMOUNT — the chip formats it through `MoneyFigure` (money is never formatted by hand) and adds "this week"; omitted when nothing is new. Never fiat.
