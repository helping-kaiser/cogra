...
```jsx
<BottomNav active="feed" slots={ALL_SLOTS} onSelect={go} /> {/* the V1.0 bar */}
```

V1.0 carries all five slots, so every board passes `ALL_SLOTS`. A slot whose surface is drawn after V1.0 keeps its place and opens a coming-soon door — `ComingSoonCard`; the wallet slot opens `WalletComingSoon` — because a thinner bar reads wrong and re-adding a slot later costs every reader the muscle memory twice. That is a slot's rule only: a kind list follows the staging rule and shows only what is served (readme §13, *The V1.0 scope cut*).
...
The discovery slot is keyed `search` — its route and its Material glyph — but its label reads **Explore**. Every tab in this bar is named for what the reader does there, and "Explore" says the same about this one: discovery through the people they're connected to, not a global index. The graph meaning belongs in a sentence on that surface, where there is room to mean it; a one-word tab naming the mechanism would be the only tab in the row describing itself differently from its neighbours.
