...
- **The seen toggle rides in the same section**, a `Checkbox` reading "Show what you've already seen", default on. Seen means the card's impression entered the viewport — device-local, never a record, shared transiently with the viewer's chosen ranker. Hiding seen is the deviation; the trigger says "hiding seen" only then.
