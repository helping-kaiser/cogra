...
- **The dot row is windowed at seven** (`ViewerDots`). Up to seven pictures the row is the set. Past that it is a window onto it: the window slides centred on where the reader is, clamped to the set's two ends, and **the edge dot on a side with more beyond it is drawn smaller** — that shrunk dot is the whole of how the row admits what it is not showing. The active dot is never the shrunk one, and every slot keeps its pitch so nothing reflows under a swipe.
