...
`DescribeCounter` is the details step's entry into per-picture descriptions — the link plus the quiet "· n of m described" count. It sits under the row, not inside it. **A video takes one description for the whole clip** (2026-09-02): pass `subject="video"` with `total={1}`. Its cover never takes a description of its own — the cover is the video's face, not a second picture.
