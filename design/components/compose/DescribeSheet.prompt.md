...
- **Over the cap, `Done` is visible-but-disabled** (backlog item 101, ruled the batch-rulings round) — the app-wide `canSign`/`canSubmit` treatment, never a hidden button and never a dialog. The refusal the reader reads is the field's own late counter, already contractual here (`TextField.prompt.md`, the capped-field rule); the closing word only stops being available. A `Done` that vanished would leave a reader hunting for the way out of a sheet they cannot leave.
