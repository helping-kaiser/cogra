...
Range `[-1, +1]`, step `0.01`, and the signed two-decimal value lives **in the label**, in a `cg-exact` span that paints only in geek mode (readme §13) — the range input announces its own value either way. Labels are always the reader's words: `For or against`, `How much reaches you` — and the ends are named too (`Against`/`For`, `Less`/`More`), because a track from −1 to +1 says nothing about which end is which.
