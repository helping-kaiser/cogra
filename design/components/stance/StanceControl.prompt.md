...
- **The `?` names its own pad** (jakob's ruling A7) — `helpLabel`, defaulting to `"How opinions work"` for the ordinary feed-card control. A board drawing a named pad passes that pad's own title instead (`ComposePad`'s `"Where you stand on it"`, `ReplyPad`'s `"Toward what you answer"`, `VouchBackPad`'s `"Your first opinion"`).
