...
Keep the fixed order: title, the pick line (only when reached by a pick), the consequences, **the raw total** ("everything you've said adds up to …"), the cap as an aside *only when the sum exceeded it*, the cost in things, then `Walk it back` / `Keep it`. The total leads and the cap is derived from it — stated the other way round it reads as broken arithmetic. Never `error` colouring — severance is a legitimate choice, not a failure. Never block the corner of the field to prevent it.
