...
Everything a post grows beyond its content goes in the **affordance row**: **one line, never wrapping, spread across the card's full width**, in a fixed order — **stance, Post score, comments, then `actions`** — with every control on a 48px target. A second row reads as a second kind of thing, and it costs height the post does not have. That constraint is why every affordance here is glyph-plus-number rather than words. Nothing in that row may take `primaryContainer` — the stance knob already spends it.
