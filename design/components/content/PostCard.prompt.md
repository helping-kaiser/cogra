...
**The Post score is a prop, not something each surface passes in.** Every post in a ranked listing has one, so `score` renders it in the affordance row after the stance control, as Material's `graph_3` glyph plus the number. Uncapped, negative allowed, never coloured. The number is drawn in both reading modes (readme §13): geek mode governs the number pairs, and a score has no glyph that could carry its magnitude. Its spoken name is `Post score`.

Everything a post grows beyond its content goes in the **affordance row**: **one line, never wrapping, spread across the card's full width**, in a fixed order — **opinion, Post score, comments, then `actions`** — with every control on a 48px target. A second row reads as a second kind of thing, and it costs height the post does not have. That constraint is why every affordance here is glyph-plus-number rather than words. Nothing in that row may take `primaryContainer` — the pad's knob already spends it.

**`redacted` renders the skeleton, not a field.** Redaction is record-granular — an illegal verdict removes the payload, so title, description, body, media, and the license go at once. Never veil or blank them individually. What survives is the point: author, timestamp, thread position, and the opinion a reader can still give, because no record leaves the graph and no removal is silent. Pass `{reason: "author"}` for a self-erasure, which must read differently from a verdict.
