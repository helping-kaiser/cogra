Use `OverflowMenu` for the interactions a reader does rarely — check a licence, report content, open a proposal against it, copy a link. `PostCard` and `CommentCard` mount one automatically and append whatever you pass in `menuItems`.
...
The dividing line: **the affordance row carries what a reader reaches for; the menu carries the rest.** A stance is the gesture the product lives on and belongs in the row. A licence is checked once in a hundred readings and belongs in here.
