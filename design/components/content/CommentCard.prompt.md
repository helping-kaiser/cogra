...
**A comment is words first and its pictures join them** (readme §13): pass `media` and the pictures render below the words, INSET at the card's medium rung — an attachment, not the body, so no full-bleed. Comment pictures are never cropped (2026-08-31); multiples ride the same `MediaGallery` pager as a post's, in a fixed square frame each whole frame fits inside (pass each item `fit: "contain"` and its true ratio). The authoring-side cap is **four pictures, or one video with its cover** — the post's own grammar at comment scale (2026-09-02). Whether a comment's video autoplays in the thread the way a post's does is not ruled yet.
