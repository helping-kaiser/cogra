...
- **`action` is one control of the row's own**, in the trailing-most slot — the chevron's, which it replaces (the Saved list's unsave, a filled bookmark with no word beside it). It does not take the trailing edge: the edge keeps its content and sits inboard, because on Saved that content is when *you* saved the thing, which is the list's whole order. Given an `action` the row splits into a pressable part and the control, so nothing nests a button in a button.
