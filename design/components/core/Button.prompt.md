...
**Both sizes are tappable at 48px.** `sm` draws 33px of ink and `lg` 40px — Material's dense and default heights — but `BUTTON_CLASS` carries `cg-hit`, which expands the target to 48px on both axes without moving the ink. So a dialog footer and the opinion pad's action row stay compact while the system's 48px minimum stays unconditional. If you build a pressable control by hand, add `cg-state cg-focus cg-hit` to it.
