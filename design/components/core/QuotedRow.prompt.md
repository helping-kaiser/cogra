...
- **The snippet ellipsizes; the title does not.** The title is a name and a handle — losing its end loses who. The snippet is a taste, and one line of it is the point. The column carries no gap because the two lines are one address, not two facts.
- The face is `MonogramAvatar`, so an author with no picture still gets one.
