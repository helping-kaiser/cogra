...
```jsx
<ExplainableNumber exact glyph="graph" label="Post Score" value="15.20" onOpenDetail={openScore} />
```
...
- **`exact` marks a SIGNAL NUMBER** (readme §13): the Post Score sets it, and its digits — the `—` placeholder included — paint only in geek mode, leaving the `graph` glyph to carry the reading. Money, ages and counts never set it; the figure is spoken in both modes either way.
