...
Copy rules: state the fact, offer the one action that changes it, and stop. `Nothing here yet.` on a profile chronicle, `No comments yet.` on a thread, `Checking your opinion…` while an opinion loads. Never "You haven't posted anything!", never a call to action for something the reader cannot do here, never `error` colour — an empty list is not a fault.
