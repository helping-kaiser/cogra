// Generates the per-canvas seed manifests (backlog item 69): a tree is one
// master, but the editor it is reviewed in caps a canvas at 200 files and 16MB,
// so review is split across the canvases the tree's `canvases.json` names.
// Membership is derived, never declared — a board belongs to the canvas that
// serves its `page`, so a new board joins one the moment this runs. Each canvas
// gets `<tree>/canvases/<id>/canvas.json` (its pages' artboards and
// annotations, coordinates verbatim) and `images.json` (the photographs its
// boards actually reference), so seeding reads one directory and scans nothing.
//
// Every generated tree is seeded, `trees.mjs` being the one place they are
// named; each tree carries its own canvases and its own budgets.
//
...
function genCanvases(treeDir) {
  const dir = resolve(here, "..", treeDir);
  ...
  if (!canvases.length) treeFails.push("canvases.json declares no canvases");

  const seenId = new Set();
  const claimedBy = new Map();
  for (const c of canvases) {
    if (!c.id || !c.title || !Array.isArray(c.pages) || !c.pages.length || !c.launch)
      treeFails.push(`canvases.json: ${c.id ?? "(unnamed)"} needs id, title, a non-empty pages list and launch`);
    if (c.title && /[<>&"\\]/.test(c.title))
      treeFails.push(`canvases.json: ${c.id}'s title carries one of < > & " backslash, which the canvas editor refuses at seed time`);
    if (seenId.has(c.id)) treeFails.push(`canvases.json: ${c.id} is declared twice`);
    seenId.add(c.id);
    for (const p of c.pages ?? []) {
      if (!pageIds.includes(p)) treeFails.push(`canvases.json: ${c.id} claims page "${p}", which canvas.json does not know`);
      if (claimedBy.has(p)) treeFails.push(`page "${p}" is claimed by both ${claimedBy.get(p)} and ${c.id} — a page belongs to exactly one canvas`);
      else claimedBy.set(p, c.id);
    }
    if (c.launch && !(c.pages ?? []).includes(c.launch)) treeFails.push(`canvases.json: ${c.id} opens on "${c.launch}", which is not one of its pages`);
  }
  for (const p of pageIds) if (!claimedBy.has(p)) treeFails.push(`page "${p}" (${pageName(p)}) is claimed by no canvas — it would be reviewed nowhere`);
  ...
  for (const a of canvas.artboards) if (!pageIds.includes(a.page)) treeFails.push(`artboard ${a.file} names page "${a.page}", which canvas.json's pages list does not declare`);
  for (const a of canvas.annotations ?? []) if (!pageIds.includes(a.page)) treeFails.push(`annotation ${a.id} names page "${a.page}", which canvas.json's pages list does not declare`);
  ...
  for (const a of canvas.artboards) {
    ...
    if (!existsSync(path)) {
      treeFails.push(`artboard ${a.file} is in canvas.json but not in the tree`);
      continue;
    }
  ...
  for (const c of canvases) {
    ...
    if (files > FILE_BUDGET) treeFails.push(`${c.id} seeds ${files} files, over the ${FILE_BUDGET} budget (the editor caps a canvas at ${FILE_CAP}) — split a page off it`);
    if (total > BYTE_BUDGET)
      treeFails.push(`${c.id} seeds ${(total / 1048576).toFixed(2)} MB, over the ${(BYTE_BUDGET / 1048576).toFixed(0)} MB budget (the editor publishes at most ${(BYTE_CAP / 1048576).toFixed(0)} MB) — split a page off it`);
    if (!artboards.length) treeFails.push(`${c.id} would seed no boards`);
  }
  allRows.push(...rows);
  ...
  for (const [rel, body] of outputs) {
    ...
    drift.push(had === null ? `${treeDir}: ${rel} was missing` : `${treeDir}: ${rel} changed`);
  }
  for (const rel of existing)
    if (!outputs.has(rel)) {
      rmSync(join(outRoot, rel), { recursive: true, force: true });
      drift.push(`${treeDir}: ${rel} no longer belongs to any canvas`);
    }
...
for (const treeDir of TREES) genCanvases(treeDir);
...
console.log(`gen-canvases: ${allRows.length} canvases ok in ${Date.now() - t0} ms`);
