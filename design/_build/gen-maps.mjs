...
//
// Every generated tree is mapped, `trees.mjs` being the one place they are
// named; each tree's maps are drawn from its own canvas and its own graph.
//
...
<div style="position:relative;flex:1;">${cardHtml}</div>`;
  ...
  for (const p of canvas.pages) {
    ...
    count += 1;
    console.log(`generated ${treeDir}/${name}.dc.html (${width}×${height})`);
  }
...
let total = 0;
for (const treeDir of TREES) total += genMaps(treeDir);
console.log(`${total} maps generated in ${Date.now() - t0} ms`);
