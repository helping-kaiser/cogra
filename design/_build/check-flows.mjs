...
//
// Every generated tree is checked, `trees.mjs` being the one place they are
// named. A tree is checked against its own canvas, its own graph and its own
// flow layer — no tree's reachability is ever answered with another's boards.
//
...
function checkTree(treeDir) {
  const dir = join(root, treeDir);
  ...
  const infos = [];
  const gaps = [];
  const say = (text) => lines.push(`${treeDir} · ${text}`);
  const fail = (text) => fails.push(`${treeDir}: ${text}`);
  ...
  for (const p of wired) if (!pageIds.has(p)) fail(`graph.json wires page "${p}", which canvas.json does not list`);
  for (const b of Object.keys(graph.boardKinds ?? {})) if (!boards.has(b)) fail(`graph.json boardKinds names "${b}", which is not a board`);
  for (const b of Object.keys(graph.entries ?? {})) if (!boards.has(b)) fail(`graph.json entries names "${b}", which is not a board`);

  // canvas <-> files drift.
  for (const a of canvas.artboards) if (!existsSync(join(dir, a.file))) fail(`canvas.json lists ${a.file}, which does not exist`);
  for (const f of readdirSync(dir)) {
    if (f.endsWith(".dc.html") && !canvas.artboards.some((a) => a.file === f)) fail(`${f} is not on the canvas`);
  }
  ...
  for (const e of graph.edges ?? []) {
    ...
    if (e.kind === undefined) fail(`edge ${key} declares no "kind" — every edge carries one of ${KINDS.join(" / ")}`);
    else if (!KINDS.includes(e.kind)) fail(`edge ${key} declares kind "${e.kind}", which is not one of ${KINDS.join(" / ")}`);
    else census[e.kind] += 1;
    if (!boards.has(e.from)) { fail(`edge ${key} starts on unknown board "${e.from}"`); continue; }
    if (!Number.isInteger(e.via) || e.via < 1) fail(`edge ${key} has a non-positive via — data-flow numbers are 1..n per board`);
    if (edgeByVia.has(key)) fail(`two edges share ${key} — one element, one edge (list outcomes in "to")`);
    edgeByVia.set(key, e);
    if (!Array.isArray(e.to) || e.to.length === 0) { fail(`edge ${key} has no outcomes`); continue; }
    for (const o of e.to) {
      const ways = ["board", "terminal", "gap"].filter((k) => o[k] !== undefined);
      if (ways.length !== 1) { fail(`edge ${key}: each outcome is exactly one of board/terminal/gap`); continue; }
      if (o.board !== undefined && !boards.has(o.board)) fail(`edge ${key} points at unknown board "${o.board}"`);
      if (o.terminal !== undefined && !(graph.terminals ?? {})[o.terminal]) fail(`edge ${key} points at undeclared terminal "${o.terminal}"`);
      if (o.gap !== undefined) gaps.push(`${key}${o.case ? ` (${o.case})` : ""}: ${o.gap}`);
      if (o.info !== undefined && (o.info !== true || o.terminal === undefined)) {
        fail(`edge ${key}: "info": true marks a terminal outcome that only tells you something — it belongs on no other shape`);
      }
    }
    ...
    if (e.kind === "advance" && e.to.every((o) => !reaches(o))) {
      fail(`edge ${key} is "advance" but every outcome only informs — an advance reaches a board, a gap, or a terminal that does something; this one is a "detour"`);
    }
  }
  ...
  for (const [name, info] of boards) {
    ...
    for (const [, n] of html.matchAll(/data-flow="(\d+)"/g)) {
      ...
      if (!edgeByVia.has(`${name}/${n}`)) {
        if (onWired) fail(`${name}: data-flow ${n} has no edge in graph.json`);
        else infos.push(`${name}: data-flow ${n} has no edge in graph.json`);
      }
    }
    for (const key of edgeByVia.keys()) {
      const [b, n] = key.split("/");
      if (b === name && !seen.has(n)) fail(`${name}: edge ${key} exists but no element carries data-flow="${n}"`);
    }
    ...
    if (onWired && info.boardKind === "screen") {
      if (!(graph.scanExempt ?? {})[name]) {
        for (const m of html.matchAll(/<(?:button|input|textarea|select|a href=)[^>]*>/g)) {
          ...
          fail(`${name}: untagged interactive element ${tag.slice(0, 80)}`);
        }
      }
      if (!(graph.entries ?? {})[name] && !inbound.has(name)) {
        fail(`${name}: no entry point — no edge lands here and it is not a declared entry`);
      }
  ...
  if (existsSync(flowsFile)) {
    ...
    for (const f of flowFails) fail(f);
    ...
    if (rebless) {
      ...
      fail(`flows.resolved.json is missing — resolve and bless it with: node check-flows.mjs --rebless`);
      ...
      fail(`flows.resolved.json no longer matches the graph:\n${moved.map((m) => `  ${m}`).join("\n")}\n  Re-bless deliberately: node check-flows.mjs --rebless`);
    }
  } else {
  ...
  say(`graph: ${(graph.edges ?? []).length} edges · wired pages: ${wiredNames} · ${gaps.length} gaps · ${infos.length} boards pending`);
  say(`kinds: ${KINDS.map((k) => `${k} ${census[k]}`).join(" · ")}`);
  for (const l of flowLines) say(l);
  if (gaps.length) {
    say("gaps (designs still owed):");
    for (const g of gaps) say(`  ${g}`);
  }
}

for (const treeDir of TREES) checkTree(treeDir);
