//! The two-pass run: the harvest, the resolution, and the judgments.
//!
...
use crate::adopt::{Adoption, Kind, OwnerId, ProfileId};
use crate::carrier::{SourceFile, Walk};
use crate::diag::{ByteSpan, Diagnostic, Enforcement, RuleId, Severity};
use crate::error::{RunError, WalkError};
use crate::frontend::{Parsed, Region};
use crate::graph::{
    AssetNode, CitationNode, Corpus, EdgeW, HeadNode, LabelNode, MintNode, NodeW, OwnerNode,
    PairNode, PairOrigin, PresentedSet, ProfileNode, RegionNode, Registries, SourceNode, owner_of,
};
use crate::judge::kinds::KindRegistry;
use crate::pretokenize::{PreTokenized, located, pretokenize};
use crate::scan::{
    Delimiter, Label, NearMiss, NearMissKind, Occurrence, Prefix, RegionScan, Syntax, scan_code,
    scan_prose,
};
use crate::timing::{Phase, Timing};
use crate::{HeadVerdict, bans, frontend, judge};
...
/// let checked = run::check(&adoption, Path::new("."))?;
///
