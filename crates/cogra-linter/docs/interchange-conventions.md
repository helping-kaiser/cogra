# The Interchange Conventions · `spec:interchange:interchange-conventions`

_A model-theoretic statement_

_Provenance: copied on 2026-08-20 from the L1 author's linter workspace, where it is design record 013, and amended in this repository since. That workspace's code is AGPL-3.0-only; its design records state no license of their own._
...
For an assigned theory S, the _open companion_ Open(S) is S with two relaxations and a guard: the minor position of key 1 is freed to uint; the closure is replaced by the base theory's wildcard, admitting any value at unknown content keys (`schema:interchange:global`); and each enumerated content key is matched with a cut (RFC 8610 §3.5.4), so a member present under a name S enumerates is held to S's type there rather than falling through to the wildcard. The two relaxations widen the model class; the cut does not — it is what keeps the wildcard from widening a known key, where an optional key carrying a value outside its type would otherwise be readmitted as `any`. Nothing else moves: every content key S names keeps its type and its requiredness. Open(S) is derived, never assigned — it enters no registry, and there is nothing in it to revise, it being a function of the immutable S. The companion is the reader's instrument for stamps above its floor (`def:interchange:acceptance`), and it is sound there by (`metathm:interchange:forward-compatibility`).
