...
A `Label` node exists once per owner that carries the label, never once per corpus: two owners minting one label text is expressly not a collision (`[LBL-cav:labels:coexistence]`), and one shared node would make it one. The `Source` node's `language` is `Option` because the carrier contains files no frontend reads — the types `[scanned-regions]` declares in its `none` rows (`dec:lint:catalogue-totality`), and whatever the working notes hold — and those files are in the carrier and owned, carrying no occurrences and vacuously in good standing (`[LBL-judg:labels:minting]`). Representing them as sources without a language is what keeps R17's walk honest; dropping them would make the partition's totality unobservable.
...
**Decision (The carrier is the tracked corpus)** · `dec:lint:tracked-carrier`

The carrier's base set is what `git ls-files` lists at the root — the files the repository's index holds — minus `[carrier]`'s literal exclusions. The calculus fixes the carrier as every committed source, excluding build and dependency directories and uncommitted generated artifacts (`[LBL-judg:labels:minting]`), and version control is the one authority on what is committed. An untracked file is therefore outside the carrier rather than inside it and excused: no exclusion row has to name a build tool's output before a local build stops tripping the check, and the verdict is a property of the commit rather than of whoever ran it. The roots `[partition]` marks `optional` are the one exception, walked on disk under the carrier module's link policy, because the working notes are gitignored by design and checked all the same (`[LBL-ansatz:labels:unchecked-locals]`). A root git will not list is a traversal failure carrying git's own message, never an empty carrier (`[LBL-cav:labels:coexistence]`), and a tracked link or submodule is read off its index mode and contributes nothing. The listing is transplanted from the L1 author's linter (orchestration-linter 0.1.0, commit 416b136), whose `git-tracked` universe answers the same question.

**Decision (The checkout is held to the tracked corpus)** · `dec:lint:tracked-exclusive`

A tracked base answers which files are the corpus and leaves two questions about the checkout open: whether it holds something the index does not, and whether it lacks something the index holds. Left open, a new source nobody added is silently outside the check, and a tracked file gone from disk is at most a read failure, so two machines checking one commit can report two different numbers without either saying why. `[carrier] universe = "tracked-exclusive"`, the L1 author's universe answer of the same name (orchestration-linter 0.1.0, commit f6b263a), asks both. A path the checkout holds that git does not track and no declaration answers for is `carrier-untracked-path`, one finding per region, because git lists a directory it holds nothing under as one entry. A tracked path the checkout lacks is `carrier-missing-tracked-path`, reported once: the path stays in the carrier, owned and unread, so the count does not move with it and no reader repeats the repair as an unreadable source. The declarations are the carrier's literal rows, the optional roots, and the repository's committed ignore files, which git applies to its own listing. The last is the adaptation: upstream holds every untracked path to its own ignore rows, and the local material a checkout gathers — `.env*`, `*.local`, per-module `build/` — is only expressible as the patterns this adoption data refuses (`[ARCH-dec:linter:no-regex]`). `[[carrier.ignore]]` rows are the upstream ignore relation in literal terms: a unique name in the label name grammar and a path, removed as a union wherever the carrier is read. Enforcement is the region's path against the failing set, and a failing disagreement refuses the regeneration like every carrier finding. `git-tracked`, the answer when the key is absent, asks neither question.

**Decision (The working notes are counted apart)** · `dec:lint:notes-apart`

The roots `[partition]` marks `optional` are walked on disk and checked (`[LBL-ansatz:labels:unchecked-locals]`), and they exist on some machines and not in a clean clone. Folded into the run's one count, they made that count a property of the machine: a checkout carrying the notes, with unpacked archives among them, reported thousands of findings a clean clone of the same commit did not. The check therefore prints the corpus's count — its sources and its findings by enforcement half — and, where a run walked any notes, theirs on a line of its own. Nothing is exempted: the notes are read and judged as before, their findings are listed with `--advisory`, and a failing one still fails the run. A finding counts where its primary location lies, and a finding with no location is the corpus's.

**Decision (Every tracked file type is scanned or declared)** · `dec:lint:catalogue-totality`

`[scanned-regions]` is a total catalogue over the tracked carrier: every file type in it is read by a `language` row or declared by a `none` row, and a type in neither is `carrier-uncatalogued-type`, one finding per type at its first file. Without the check a new kind of file enters the carrier scanned by nothing and declared by no one, which is how the web tree's `.mjs` and `.mts` sources sat unread under a ruling of "comments, all forms". The split follows one rule: a comment-bearing type is scanned wherever a frontend reads it, and declared otherwise, in one of two rows — comment-bearing formats awaiting a frontend, since (`[ARCH-dec:linter:ast-frontends]`) forbids reading them as raw text, and formats with no comment form at all. A type a frontend could read but whose files are not yet swept for the scanned-code label syntax is declared too, in a row of its own saying so. A `none` row names a type by extension or, for the types no extension names, by whole file name — the selector shape of the L1 author's comment-leader catalogue. The finding fails the lane wherever its file sits, because the defect is the catalogue's and not the file's. The working-note roots are outside the question: they are no commit's content.
...
The test profile's recognizer is "any attribute path whose final segment is `test`", which is the open rule `[profiles]` states so that a fourth harness needs no code change; the module profile's census counts module *definitions* — an inline `mod name { ... }` or the file backing a `mod name;` — once per definition and never per declaration, excluding `#[cfg(test)]` modules. Both are `[profiles]` data read by one walk, and a staged profile's census stays out of `Parsed::assets` while it waits (`dec:lint:staged-profiles`), so entering Π flips fields rather than writing code. A `mod name;` declaration is neither a definition nor an asset, and the frontend reports it separately — the definition backing it is another file, and pairing the two is a cross-source step no frontend can take with one source in hand (`dec:lint:cross-source-pairing`).
...
**Decision (The declaration–definition pairing is one step, shared)** · `dec:lint:cross-source-pairing`

A definition-recognized census is not a per-source quantity: the file backing a `mod name;` holds the definition and the declaring file holds the name, so how many definitions a corpus has is knowable only to a run holding every source. The unresolved half leaves the Rust frontend as a `Declaration` — in `Parsed::declarations` on the parse path and in `Censuses::declarations` on the census path — and `frontend::backing_definitions` completes it. Cargo's own layout is the rule: a declaration in a crate root or a `mod.rs` is backed from that file's own directory, one in any other module file from the directory named after it, and a declaration whose backing file is outside the carrier pairs with nothing. Each backing source yields one asset however many declarations name it — once per definition, never per declaration — and the result is ordered by the backing source's path (`[ARCH-req:linter:determinism]`). The roots the rule knows are `lib.rs`, `main.rs`, `mod.rs`, and every file directly under a package's `tests` directory. Those last are crate roots by being a target's own entry point: "Files located under the `tests` directory are integration tests", and "Cargo will compile each of these files as a separate crate" (The Cargo Book, *Cargo Targets*, verified 2026-08-26). So the shared tree the Book's own idiom puts beside them — `tests/common/mod.rs`, declared `mod common;` in every suite — is one definition however many suites declare it, which is exactly what the definition census asks for. Which directory is a package's `tests` comes from the frontend's one reading of the Cargo layout, the same one the test profile's area derives through, so a `tests` directory inside a `src` tree belongs to the lib target and roots nothing.

There is one implementation, and it lives in `frontend.rs` because its input and its output are that module's own types and because the two runs that need it would otherwise each grow one. The measurement calls it while computing a staged profile's census (`dec:lint:migrations-subcommand`); the judging run calls it once, at the close of pass 1, so a paired asset and a source's own are the same thing to everything downstream. Two censuses of one profile over one corpus that disagreed would put the inventory judgment over an asset set the measurement never reported — a bijection checked against the wrong domain, which no diagnostic would name (`[LBL-inv:labels:inventory]`).

The step closes pass 1 rather than opening pass 2, and the line matters. Pass 2 adds the edges that judge completed registries and adds no nodes (`[ARCH-rule:linter:two-pass]`); a file-backed definition is a census fact, settled by the corpus's declarations alone and before any registry is read, so it belongs to the pass that harvests.
...
**Decision (Regeneration refuses to write over a carrier that disagrees with the adoption data)** · `dec:lint:regenerate-precondition`

`regenerate` runs the check before it writes, and refuses the write while the failing set holds a finding of the carrier module: a configured root the walk found nothing under, or a tree or file it could not read. Each of those says the adoption data and the tree disagree, so the registers would be generated from a carrier the run could not account for, and a writer that proceeded would record as current a conclusion drawn from a question the corpus had not answered. The rule is adopted from the same sibling linter as (`dec:lint:fix-subcommand`), whose every writing mode refuses under a declaration that disagrees with the tree.

Every other finding leaves the write alone, and that line is the rule's content rather than a leniency. A stale register fails the check, and regeneration exists to repair exactly that; a label defect is its author's to fix, and a register generated beside it is still the one generator's output over the carrier as read. The sibling draws the line in the same place: its guard judges the partition, the universe, and the dependency closure, and no label or projection finding reaches it. The disagreements nameable without walking the tree — a configured path spelled otherwise than the root spells it, a package with no partition rule, a reach row a manifest contradicts — are already errors of the run entry here (`crit:lint:error-or-finding`), so a regeneration never gets past them; the guard covers the half only the walk can find.

The refusal takes the class of (`dec:lint:fix-precondition`): the disagreeing findings go to stderr, stdout stays empty, nothing is written, and the exit is `2`. `--dry-run` writes nothing and is never refused, which is how an operator sees what a repaired carrier will change. Only the failing set counts, because the enforcement partition decides what fails, and a guard that weighed an advisory finding would be a warning deciding an exit code. The named regeneration of (`dec:lint:staged-profiles`) runs no check, and the guard does not reach it.
...
`check --advisory` lists the advisory half; without it, advisory findings are counted in the summary and not listed, so a run that is clean on the failing set reads as clean. The summary counts the corpus, and the working notes on a line of their own where the run walked any (`dec:lint:notes-apart`).
...
The measurement walks the corpus itself rather than reading one `Parsed`, because the module profile's distance counts the definitions backing its `mod name;` declarations and those lie in other files. It does not own that step: the pairing is one implementation and the judging run calls the same one (`dec:lint:cross-source-pairing`), so the census a measurement reports and the census a check judges cannot drift apart. What it looks at, it says: a definition counts as arrived when its own source carries a mint of the derived label inside an inner documentation comment, and that the comment opens the module's body is not checked.
...
| tree-sitter | 0.26.13 | 2026-08-26 | docs.rs/tree-sitter | runtime | the Kotlin frontend runtime (`[ARCH-dec:linter:kotlin-tree-sitter]`) |
...
Versions that have moved since the architecture's table of 2026-08-20 are recorded rather than absorbed. syn stands at 3.0.4 where the architecture pinned 3.0.3; the pin is a floor and Cargo's caret resolves upward, so nothing is owed but the note. `toml` stands at 1.1.4, a crate the architecture's table does not carry at all, because the adoption-data parser is this document's decision (`dec:lint:toml-parsing`). swc arrives as three crates where the architecture's table names one: the parser takes its comments store and its source map from `swc_common` and its parse target from `swc_ecma_ast`, and all three release on one train — 45.0.0, 26.0.0 and 29.0.0 were published within a day of each other and verified together. The caret resolves the parser to 45.1.0 today, which is the floor behaving as syn's does. tree-sitter stands at 0.26.13 where the architecture's table names the 0.25 line, and the movement is the one the table's own re-verification clause anticipated: the runtime is pinned to the release whose CLI generated the committed `parser.c`, which the grammar's build log records as 0.26.13, so generator and runtime come from one release rather than two. The parser declares ABI 14 and the runtime admits it; the suite asserts that rather than assuming it. Two crates join it that the architecture's table does not name — `tree-sitter-language`, because the vendored tree carries no `bindings/rust` and this crate therefore writes the binding itself, and `cc`, which is a build dependency and the only one the crate has.
...
*`walkdir` or `ignore`.* The carrier is the tracked set git lists (`dec:lint:tracked-carrier`), cut by literal path prefixes in the adoption data — a prefix ending in `/` matches a tree, a prefix naming a file matches that file, and there is no pattern dialect (`[ARCH-sig:linter:adoption-data]`). `ignore` would bring glob semantics and `.gitignore` resolution into the crate, a pattern dialect entering by the back door, to answer a question git already answers; `walkdir` brings ordering and symlink policy the design must fix itself anyway for the optional roots it walks, since the walk sorts by path for determinism. A recursive `std::fs::read_dir` with sorted entries is some dozens of lines and is the thing the specification describes.
...
(`req:lint:timing`) and (`[ARCH-req:linter:timing]`) require a budget beside every recurring action; a recurring action with no budget is itself a defect. The numbers below are measurements, taken on 2026-08-27 over the 952-source carrier in the `claude-cogra` toolbox with the worktree on `/mnt/c`, debug build, warm. Each is the range of repeated runs, and each budget sits just above it. Exceeding a budget is a finding, not a cost to absorb.

| Action | Budget | Tolerance | Measured |
| --- | --- | --- | --- |
| full-corpus `check`, warm | 6.5 s | +50% | 5.74–6.10 s |
| — the walk (pre-tokenize and harvest) | 6.0 s | +50% | 5.49–5.85 s |
| — the analysis (resolve and judge) | 320 ms | +100% | 253–278 ms |
| per-phase report | every phase named and timed | — | (`[ARCH-req:linter:timing]`) fixes the five phases |
| the crate's suite (`cargo test -p cogra-linter`) | 48 s | +50% | 39.5–44.7 s |
| — the four corpus-reading binaries | 28 s | +50% | 25.2–27.0 s |
| — the property lane at 256 cases | 1 s | +50% | 0.78–0.85 s |
| the linter's addition to `make ci` | 6.9 s | +50% | 6.36–6.44 s |

The walk is timed apart from the analysis because the two grow for different reasons and a single total hides both. The walk is the whole cost — reading 952 sources and parsing them through the four frontends — and it scales with the corpus; the analysis over the finished graph remains over an order of magnitude smaller, so a resolution or judgment that turned superlinear would stay invisible inside a total the walk dominates. That is the regression the split exists to catch.

The suite splits on the same principle. The property lane is timed on its own because a case count raised without notice is exactly the regression the rule exists to catch — it holds under a second, and an explicit `PROPTEST_CASES=256` measures the same, which confirms the default the framework documents. What dominates the suite is instead the four binaries that read the real tree rather than a fixture — `corpus_acceptance`, `migrations`, `registers`, `registry_as_data` — at some 26 s of the 41, because three of them run their own full check. That concentration is why the pipeline runs this suite in the corpus lane rather than behind the Rust gate, which the commissioning note records.

Two figures locate the budget in its environment rather than treating it as portable. The same warm check on the distro's native filesystem runs in 3.65 s, so a bit over a third of the recorded budget above is `/mnt/c` crossing rather than work. And the release binary runs it in 0.63 s natively against debug's 3.65 s: the walk is dominated by parsing, which optimizes by about 6×. The lane still builds debug, because it compiles the crate in debug for the suite regardless and a release profile would be a second build and a second cache for three seconds of runtime.
