    ...
    let rows = sqlx::query!(
        "INSERT INTO auth_applicants
             (id, invite_link_id, handle, email, password_hash,
              ...
              actor_pubkey, realization_address, expires_at)
         ...
         ON CONFLICT (email) DO UPDATE SET
             ...
             realization_address = EXCLUDED.realization_address,
...
pub async fn applicant(pool: &PgPool, id: Uuid) -> Result<Option<Applicant>, sqlx::Error> {
    Ok(sqlx::query!(
        "SELECT id, invite_link_id, handle, email, password_hash,
                email_verified_at, actor_pubkey, realization_address,
...
pub async fn land_applicant(
    ...
    let row = sqlx::query!(
        "SELECT handle, email, password_hash, actor_pubkey, realization_address
         FROM auth_applicants
...
pub async fn inviter_of(
    ...
    Ok(sqlx::query!(
        "SELECT i.id, i.kind, i.handle, i.actor_pubkey, i.realization_address
...
pub async fn actor_identity(pool: &PgPool, id: Uuid) -> Result<Option<ActorIdentity>, sqlx::Error> {
    Ok(sqlx::query!(
        "SELECT id, kind, handle, actor_pubkey, realization_address FROM actors WHERE id = $1",
        id,
    )
    .fetch_optional(pool)
    .await?
