...
pub async fn prepare_stance<B: L1Boundary>(
    ...
    let (net_d, net_i) = mirror::net_opinion(
        pool,
        &NodeId::Addr(author.realization_address.clone()).to_string(),
        &NodeId::Prof(target_actor.realization_address.clone()).to_string(),
    )
    .await
    ...
    Ok(prepare::prepare(
        ...
        Gesture {
            author: author.realization_address,
            family: Family::Opinion,
            middle: None,
            target: NodeId::Prof(target_actor.realization_address),
            p_d: delta_d,
            p_i: delta_i,
            settlement_ref: None,
            license: None,
