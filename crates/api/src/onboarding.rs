...
pub struct ApplicationInput {
    ...
    pub realization_address: String,
}
...
pub async fn submit_application(
    ...
    // The key ceremony's outputs must cohere: the address is the one
    ...
    if crypto::address_of(&verifying) != input.realization_address {
...
async fn approve_one<B: L1Boundary>(
    ...
    let opinion = prepare::prepare(
        ...
        Gesture {
            author: inviter_identity.realization_address,
            family: Family::Opinion,
            middle: None,
            target: NodeId::Prof(approved.realization_address.clone()),
            p_d: approval.p_d,
            p_i: approval.p_i,
            settlement_ref: None,
            license: None,
...
pub async fn ensure_admission_staged<B: L1Boundary>(
    ...
    let balance = boundary
        .balance(&applicant.realization_address)
        .await
        .map_err(|e| OnboardingError::Internal(e.to_string()))?;
    if balance.burned_total == 0.0 {
        funding
            .credit_burn(&applicant.realization_address, cfg.admission_burn_micro)
            .await
            .map_err(|e| OnboardingError::Internal(e.to_string()))?;
    }
    ...
    Ok(prepare::prepare(
        ...
        Gesture {
            author: applicant.realization_address.clone(),
            family: Family::Registration,
            middle: None,
            target: NodeId::Prof(applicant.realization_address.clone()),
            p_d: 1.0,
            p_i: 1.0,
            settlement_ref: None,
            license: None,
