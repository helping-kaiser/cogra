...
impl User {
    ...
    async fn landing(&self, ctx: &Context<'_>) -> async_graphql::Result<Landing> {
        let Some(address) = self.identity.realization_address.as_deref() else {
            return Ok(Landing {
    ...
    /// The account's attached address, null before the key ceremony.
    /// Field-level: viewer-only.
    async fn realization_address(&self, ctx: &Context<'_>) -> Option<String> {
        if !self.is_viewer(ctx) {
            return None;
        }
    ...
    async fn has_reciprocated(&self, ctx: &Context<'_>) -> async_graphql::Result<bool> {
        ...
        let (Some(viewer_address), Some(inviter_address)) =
            (&self.identity.realization_address, &inviter.realization_address)
        else {
            return Ok(false);
        };
...
/// The requesting viewer's address, when they have one. Pending rows
...
/// the bytes, never supplied — `durationMs` reads a video's length, an
/// animated still's when its frames state one, and null on a single-frame
/// picture.
