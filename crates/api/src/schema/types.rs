...
impl HashtagType {
    ...
    async fn tagged_content(
        ...
        for row in rows {
            // A claim whose node CoGra carries no display row for is
            // nothing this surface can render — the fold is over the
            // mirror, which reaches further than the display store.
