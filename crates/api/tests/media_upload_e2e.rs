...
impl Rig {
    ...
    async fn upload(&self, token: &str, alt_text: Option<&str>, file: &[u8]) -> Value {
        let operations = json!({
            "query": UPLOAD_MEDIA,
            "variables": { "input": { "file": null, "altText": alt_text }},
        })
        .to_string();
...
async fn an_upload_stores_stripped_bytes_under_their_own_digest(pool: PgPool) {
    ...
    assert_eq!(media["altText"], "A sunset", "alt text is trimmed");
...
async fn re_uploading_the_same_picture_returns_the_same_asset(pool: PgPool) {
    ...
    assert_eq!(
        second["media"]["altText"], "Once",
        "the row is immutable — a retry does not rewrite it"
    );
...
async fn a_file_that_is_not_a_webp_image_is_refused_by_field(pool: PgPool) {
    ...
    let refused = rig.upload(&token, None, &png).await;
