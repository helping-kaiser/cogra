...
async fn fresh_bootstrap_creates_both_halves(pool: PgPool) {
    ...
    for family in [Family::Publish, Family::Tag] {
        assert!(
            mirror::has_record_by(&pool, &publisher.realization_address, family)
                .await
                .expect("gate"),
    ...
    assert!(
        mirror::has_record_by(&pool, &operator.realization_address, Family::Registration)
            .await
            .expect("gate")
    );
    ...
    assert_eq!(
        ids[0],
        format!("act:{}:0:registration", operator.realization_address)
    );
}
...
async fn crash_before_the_l1_half_is_repaired(pool: PgPool) {
    ...
    let publisher = genesis::actor_by_handle(&pool, genesis::PUBLISHER_HANDLE)
        ...
        .expect("row");
    assert!(
        mirror::has_record_by(&pool, &publisher.realization_address, Family::Publish)
            .await
            .expect("gate")
    );
}
