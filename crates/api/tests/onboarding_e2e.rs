    ...
    let attached = rig
        .gql(
            ...
            json!({ "input": {
                ...
                "realizationAddress": joiner_key.address(),
            }}),
        )
        .await;
...
/// `realizationAddress` carry the account's own attached key for its viewer,
...
async fn the_attached_key_reads_for_its_viewer_only(pool: PgPool) {
    ...
    let me = rig
        .gql(
            Some(&inviter_token),
            "{ me { actorPubkey realizationAddress } }",
            json!({}),
        )
        .await;
    assert_eq!(
    ...
    assert_eq!(
        me["me"]["realizationAddress"].as_str().expect("address"),
        inviter_key.address()
    );
    ...
    let me = rig
        .gql(
            Some(&joiner_token),
            "{ me { actorPubkey realizationAddress invitedBy { ... on User { actorPubkey realizationAddress } } } }",
            json!({}),
        )
        .await;
    assert!(
