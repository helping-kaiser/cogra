...
CREATE TABLE staged_writes (
    id             UUID        PRIMARY KEY,
    -- The staging actor. An applicant's staged Registration stages under
    -- their own actor row like any other write — the account exists from
    -- registration (auth.md §Application).
...
CREATE INDEX staged_writes_actor_idx     ON staged_writes (actor_id);
