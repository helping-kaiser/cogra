...
- **Email recovery restores the login** — the server-side
  ...
  address, is client-held and CoGra cannot reissue it.
...
- **A displayed code is dismissed by the code itself.** Every
  ...
  below). A copy button sits alongside it, and its
  clip carries `EXTRA_IS_SENSITIVE` so a paste preview masks the
  secret rather than rendering it; the copy fills the clipboard,
  the paste is the answer. What this proves is that the code
...
address on L1 even if CoGra disappears. So settings reveals
...
3. **The key ceremony, as a logged-in step.** The app generates
   the applicant's signing key and address locally and
