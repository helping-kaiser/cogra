...
| `moderation_status` | String | `'normal'` / `'sensitive'`. Layered. Default `'normal'`. Set via Network governance, or auto-flipped to `'sensitive'` by the system when any field on the node receives a redaction marker — see [moderation.md](../instances/moderation.md). |
...
`moderation_status` (`'normal'` / `'sensitive'`, layered,
default `'normal'`, auto-flipped on field redaction — see
