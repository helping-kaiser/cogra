...
`:Proposal` carries one per-field moderation surface:
`proposed_value_status`, the layered companion for its single
user-bearing field
([proposal.md §2](../instances/proposal.md#2-graph-side-properties)).
On a passing `'illegal'` classification the cascade writes the
redaction marker onto `proposed_value` in place (identity
properties don't layer) and onto the status companion. No
`moderation_status` cache — one field needs no max-severity
rollup.
