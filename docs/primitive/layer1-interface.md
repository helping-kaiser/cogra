...
| burn snapshot $B_i^{(k)}$ | gate via $\alpha$ | — | interface·input | consume only |
...
derived constants (§6 below). The operator's economic inflow is the realization's
...
averaged — no member's burn deficit is paid by another's surplus) and
...
in $B_i$. Published per-actor scalars; derivable from the public burn
...
enumerates fifteen** — these seven plus eight terminal
...
- **Transparency of Protocol Constants
  ...
  without clients being able to detect the change before it applies.
...
- **Independent Derivability of Standing Inputs
  ...
  computation — $B_i$ (public burn-primitive interface), $N_i$ (from
...
Outside this invariant (terminal or realization-side, reproduced by their own
owners): feed ranking, device-local computation scope, centrality
certificates, CAN attribution-view insulation, and the burn-primitive preservation
...
Tag semantics (`app:symbols`, `def:network:object-taxonomy`): **(interface·input)**
...
carry **(L1·closure)** except $B_i$/$B_{\max}$ **(interface·input)** and the
...
| $(1-f)\zeta$ | Realization net-live-share interface object (imported); enters only the entry cost $\delta_{\min}$. | `def:comparator:entry-cost` |
...
- **maturity** — $\tau$ is graph connection-context maturity; burn-primitive
...
$\Delta k_{\min}$ (burn primitive, `PA-`-cited) · $\Pi$ (excursion budget) vs
$\prod$ · index $a$ (an opaque address vs actor label after frame
