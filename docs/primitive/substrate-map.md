...
| Collective | gesture | **One L1 Actor + Profile**, keypair and address held in backend custody. Founding: backend creates the pair, the θ-debit is treasury-funded, founder Opinions toward the collective Profile lift its standing. See [collectives.md](../instances/collectives.md). |
...
| Reference / quote / embed / mention | payload | Reference targets live in the committed payload (witness-covered), mirrored into the overlay for traversal. The fanout budget is an L2 write-validation rule. L1's attribution never sees payload references — crediting quoted authors is CoGra's own attribution ([economics.md](economics.md)). |
...
| Concept | Primary | Mapping |
|---|---|---|
| Proposal | overlay | Proposal machinery is pure L2 — overlay Proposal node, vote edges, `:TARGETS`, role state; display text in Postgres. Exactly two L1 gestures per proposal: **creation** — the proposer authors a Content anchor node (payload = proposal text, witnessed, costs the proposer capacity); **finalization** — a system actor authors Opinion `(0,0)` + payload (outcome, tally digest) toward the anchor. Votes never touch L1. See [proposal.md](../instances/proposal.md), [governance.md](governance.md). |
| Votes, roles, quorums | overlay | Carried verbatim as L2 policy: two-tier mod-gate, petition + dual quorum, counter-Proposals, rule snapshot. |
| `:Network` singleton | gesture (system actor) | Anchored on L1 as a publisher-authored Content node — the network charter. Passed parameter changes are finalized onto the anchor as witnessed payloads (newest finalization per parameter wins), making the parameter schedule replayable from public records. The overlay singleton is the operational carrier — layered governed properties, `:TARGETS`-targetable — that the ranker and backend read. See [network.md](network.md). |
...
| Concept | Primary | Mapping |
|---|---|---|
| Admission money | consume | the burn value behind `B_i` and the θ-debit — consume-only; CoGra reads `B_i` and the entry cost through L1's interface and never authors a realization's records. |
