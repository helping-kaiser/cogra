...
| Campaigns, settlement, payouts | gesture | The campaign's public record rides L1: an advertiser-authored Content anchor carries the terms as witnessed payload, `(0,0)` References declare the targeting, and adjustments and settlement land as witnessed payloads on Opinions toward the anchor ([economics.md §3](economics.md#3-the-campaign-record), [§10](economics.md#10-the-settlement-record-and-the-claim-flow)). The money rail and payout plumbing stay CoGra's own, off-graph; the attribution they pay against is computed over L1 records (below). |
